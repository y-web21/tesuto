...
@section('content')
    <form>
        ...
        <button type="submit" id="btn_edit_confirm" formmethod="post" formaction="{{ route('post.update', ['post' => $article->id]) }}">更新</button>
        <button type="submit" id="btn_edit_image_select" formmethod="post" formaction="{{ route('post.tempsave') }}" class="btn-gray">画像を選択</button>
    </form>
